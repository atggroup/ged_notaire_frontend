(function () {
  ...
  function dossierRow(item) {
    return '<tr><td class="tname"><span class="fic" style="background:var(--primary-soft);color:var(--primary-600)"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 7a2 2 0 0 1 2-2h4l2 2.5h8a2 2 0 0 1 2 2V18a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg></span><span>' + escapeHtml(item.client || item.nom || "Sans client") + '<br><span class="ls" style="font-weight:400">' + escapeHtml(item.reference || "—") + '</span></span></td>' +
      '<td>' + escapeHtml((item.domaineLabel ? item.domaineLabel + " · " : "") + (item.objet || "—")) + '</td><td>' + escapeHtml(item.notaire || "—") + '</td><td>' + escapeHtml(item.documentsCount != null ? item.documentsCount + (item.documentsCount > 1 ? " documents" : " document") : "—") + '</td>' +
  ...
  function renderBackupTile() {
    ...
    api("GET", "/backups").then(function (data) {
      ...
      // Format court : « 24 septembre 2026 » passait sur trois lignes dans
      // la tuile, sur ordinateur comme sur téléphone.
      if (valeur) {
        var quand = derniere.createdAt ? new Date(derniere.createdAt) : null;
        valeur.textContent = quand && !isNaN(quand) ? quand.toLocaleDateString("fr-FR", { day: "numeric", month: "short", year: "numeric" }) : "—";
        if (quand && !isNaN(quand)) valeur.title = docDate(derniere.createdAt) + " à " + quand.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" });
      }
      if (tendance) {
